...
### 1.1 레이아웃 원칙 (허브 모음 뷰)
- **상단**: `DashboardHubOverview` — 참여 중인 허브 N개를 사건당 한 줄로 표시
  `사건명 — 준비 82% · 미읽음 3 · 미납 0 · 최근 12분 전`
  각 줄 클릭 → 해당 `/case-hubs/[hubId]` 상세로 이동
- **하단**: `DashboardHubClient` — 기존 카드 조합 (알림·메시지·일정·청구·팀 등
  cross-cutting). 이건 점진 deprecate 대상이지만 당장은 그대로 유지하여 기능
  회귀 방지.
- 기존의 흩어진 카드 중 허브 정보와 중복되는 것(미읽음 총합, 허브 카드 등)은
  후속 커밋에서 `DashboardHubOverview`의 요약과 통합 예정.
- 데이터 흐름: 
  `dashboard/page.tsx`가 `getCaseHubList(organizationId, 8)` + 기존 snapshot을
  Promise.all로 병렬 조회해 두 컴포넌트에 전달.
