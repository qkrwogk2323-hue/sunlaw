...
> 작성: 2026-04-16
> 지위: 직원(staff/manager) 계정의 진입 기본 페이지.
> 샘플 포맷: `docs/page-specs/notifications.md`를 따름.

## 1. 목적
조직 구성원이 로그인 직후 도달하는 **단일 요약 화면**.
여러 사건·비용·일정·알림의 **요약만** 담당하고, 실제 상태 변경은 각 처리
화면(`/notifications`, `/cases`, `/clients`, `/case-hubs`)에서 수행한다.

이 페이지는 **허브 projection(`src/lib/queries/case-hub-projection.ts`)의 읽기 전용
뷰**이다. 카드가 독자 쿼리로 사건 상태를 다시 계산하지 않는다.
