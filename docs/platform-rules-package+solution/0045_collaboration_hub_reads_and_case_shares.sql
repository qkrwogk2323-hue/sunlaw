...
using (
  ...
  or exists (
    select 1 from public.organization_collaboration_hubs hubs
    where hubs.id = hub_id
      and (app.is_org_member(hubs.primary_organization_id) or app.is_org_member(hubs.partner_organization_id))
  )
);
...
with check (
  ...
  and exists (
    select 1 from public.organization_collaboration_hubs hubs
    where hubs.id = hub_id and (hubs.primary_organization_id = organization_id or hubs.partner_organization_id = organization_id)
  )
);
...
for all to authenticated
using (app.is_platform_admin() or app.is_org_member(shared_by_organization_id))
with check (app.is_platform_admin() or app.is_org_member(shared_by_organization_id));
drop trigger if exists trg_org_collaboration_requests_updated_at on public.organization_collaboration_requests;
create trigger trg_org_collaboration_requests_updated_at before update on public.organization_collaboration_requests for each row execute procedure app.set_updated_at();
drop trigger if exists trg_org_collaboration_hubs_updated_at on public.organization_collaboration_hubs;
create trigger trg_org_collaboration_hubs_updated_at before update on public.organization_collaboration_hubs for each row execute procedure app.set_updated_at();
drop trigger if exists trg_org_collaboration_reads_updated_at on public.organization_collaboration_reads;
create trigger trg_org_collaboration_reads_updated_at before update on public.organization_collaboration_reads for each row execute procedure app.set_updated_at();
drop trigger if exists trg_org_collaboration_case_shares_updated_at on public.organization_collaboration_case_shares;
create trigger trg_org_collaboration_case_shares_updated_at before update on public.organization_collaboration_case_shares for each row execute procedure app.set_updated_at();
