...
|------|------|------|-----|
| 1 | 조직 가입 요청 → 승인 | ✅ | — |
| 2 | 사건 생성 → 허브 연동 | ⚠️ P1 | 허브 자동 생성 안 됨, 수동 연동 필요 |
| 3 | 의뢰인 초대 → 포털 활성화 | ✅ | — |
...
**P0 2건, P1 1건.** P0은 양방향 피드백 루프가 끊긴 것 — 의뢰인은 청구 사실을 모르고, 직원은 서명 완료를 모름.
...
### P1-1: 사건 생성 후 허브 안내 (단계 2)

**추가할 것:**
- `createCaseAction` 성공 후 반환되는 페이지에 "이 사건의 허브를 만드시겠습니까?" 안내 CTA
- 또는: 사건 생성 시 자동으로 case_hub도 함께 생성 (RPC 확장)
- 현재 설계가 "모든 사건에 허브가 있는 것은 아님"이라면, 안내 CTA만으로 충분

---

## 우선순위

1. **P0-1** 청구 → 의뢰인 알림 (난이도 낮음, `notifyDocumentStakeholders` 패턴 재사용)
2. **P0-2** 서명 → 직원 알림 (난이도 낮음, 같은 패턴)
3. **P1-1** 허브 생성 안내 (난이도 중, UI 변경 + 설계 결정 필요)
