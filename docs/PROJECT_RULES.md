...
1. 위 파일들은 `PROJECT_RULES.md`의 규범 부속서로서 효력을 가진다.
2. 본문 규칙과 부속서가 충돌하면 더 보수적이고 데이터 무결성에 유리한 해석을 우선한다.
3. 수식, 임계치, 로그 라우팅, 복구 순서, 검색 범위, 구독 잠금 상태머신은 부속서 원문을 기준으로 구현한다.
4. 부속서 원문은 임의로 요약본으로 대체하지 않는다.
...
```txt
플랫폼 조직 = DB canonical governance 모델이 지정하는 운영 기준 조직
...
| 플랫폼 관리자 | Platform Operator | DB canonical governance 모델과 `app.is_platform_admin()`가 판정하는 관리 주체 | `app.is_platform_admin()`, `organizations`, `organization_memberships` |
