...
모든 업무 메뉴의 로그 노출 방식은 아래 원칙을 따른다.

1. 각 메뉴에는 `작은 로그 버튼 1개`만 둔다.
2. 로그 버튼은 별도 페이지 난립이 아니라 `팝업`으로 연다.
3. 팝업에는 `현재 메뉴와 관련된 기록`만 보여준다.
4. 팝업 기록은 `현재 조직` 기준으로만 보여준다.
5. 플랫폼 관리자 전용 메뉴가 아닌 곳에서 `/admin/audit`로 이동시키면 버그로 본다.
6. 같은 내용을 다시 보여주는 `최근 기록` 카드, `중복 기록 섹션`은 두지 않는다.
...
<DangerActionButton
  ...
  confirmTitle="사건 삭제"
  confirmDescription="삭제하면 보관함으로 이동되며, 언제든 복구할 수 있습니다."
...
10. **사용자 인터페이스에 개발언어, 영문 시스템 문구, 내부 코드 용어, 개발자용 설명, 추상적인 업무 표현을 직접 노출하면 안 된다.** 꼭 필요한 경우에도 화면 문맥에 맞는 한국어 문장으로 다시 써서 표시한다.
11. **같은 의미의 상태 카드·안내 박스·요약 박스는 한 화면에서 크기와 정렬을 맞춘다.** 나란히 있는 박스 높이가 제각각이거나 문장이 깨지는 구조는 금지한다.
...
17. 모든 AI 카드/응답 컴포넌트는 AI 피드백 버튼(ThumbsDown 아이콘 + "AI 결과가 틀렸나요?" 레이블)을 제공해야 한다. "오답 신고"라는 표현은 사용하지 않는다.
18. AI 피드백은 `사용자 역할, 화면, 질문, 답변, 근거, 모델 버전, 요청 ID, 시간, 신고 사유`를 구조화 저장해야 한다.
19. AI 피드백 상태는 `접수/분석중/조치완료`로 추적 관리해야 한다.
20. 반복 피드백 유형은 규칙/프롬프트/권한 필터 개선 백로그에 자동 등록해야 한다.
...
25. 출시 승인은 `마스킹 테스트`, `권한 우회 테스트`, `출처 표시 링크`, `AI 피드백 저장/대시보드`, `감사로그 누락`, `롤백 절차 문서화`를 모두 통과해야 한다.
...
5. 플랫폼 관리자 전용 기능이 일반 사용자조직 또는 의뢰인 화면에 노출되거나 실행되면 버그로 간주한다.
6. 위 버그가 감지되면 플랫폼 관리조직에 운영 알림과 감사로그를 남긴다.
7. 플랫폼 운영 판단, 조직 승인, 구독 조정, 조직 삭제, 운영 권한, 감사로그 판단은 AI가 답하거나 제안하지 않는다.
...
5. 플랫폼 전용 메뉴 누출, 플랫폼 전용 알림 누출, 플랫폼 전용 액션 실행 시도, 설명되지 않은 주요 오류와 버그는 플랫폼 관리조직 로그 싱크에 기록한다.
6. 플랫폼 관련 질문은 AI가 답변하지 않고, 반드시 해당 운영 메뉴를 직접 안내한다.

### 5-17-1. staged 변경 보존 및 푸시 차단 대응 규칙

1. 본인이 하지 않은 staged 변경은 절대 수정, 되돌리기, 재정렬, 혼합 커밋하지 않는다.
2. 본인이 하지 않은 staged 변경 때문에 푸시가 막히면 해당 변경을 건드리지 않고 현재 작업만 별도 브랜치 또는 로컬 상태로 분리해 보존한다.
3. 이 상황에서는 아래 중 하나의 상태 문구를 사용자에게 먼저 알려야 한다.
   1. `우선 브랜치에 올렸습니다.`
   2. `우선 로컬저장된 상태이고, 푸시현황에 따라 작업이 가능합니다.`
   3. `이 변동사항을 먼저 푸시하시겠습니까?`
4. 사용자가 staged 변경 처리까지 명시적으로 지시하지 않으면, 그 staged 변경을 해결 대상으로 간주하지 않는다.
5. staged 변경이 있는 상태에서 추가 작업을 이어갈 때는 현재 푸시 가능 상태와 충돌 여부를 먼저 다시 설명한다.
