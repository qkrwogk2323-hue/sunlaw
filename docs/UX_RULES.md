...
- 폼 상단에 필수/선택 안내를 반드시 표시
- URL, 이메일, 사업자번호, 전화번호, 주민번호처럼 검증이 엄격한 필드는 형식 힌트를 같이 표시
- raw Zod/JSON/영문 검증 에러를 그대로 노출하지 않고, 사람이 읽을 수 있는 한국어 문장으로 변환

### 기능 완결성 & 알림 동선

- 화면에 보이는 숫자와 실제 항목 수는 반드시 같아야 함
- 알림, 승인 요청, 후속 처리 카운트는 눌렀을 때 실제 목록/상세로 이어져야 함
- 카드처럼 보이는 KPI/요약 카드는 실제로 클릭 가능해야 함
- 알림은 `destination_url` 없이 생성 금지
- 알림 목적지를 특정할 수 없을 때는 `/dashboard` 같은 generic fallback 금지, 이유와 대체 경로를 알려야 함
- 기능 검토는 `생성 → 승인/연결 → 알림 → 목적지 이동 → 처리 → 보관/삭제/복구 → 로그` 순서로 수행
- 사용자는 각 단계에서 어디로 가야 하는지, 왜 가야 하는지, 못 가면 왜 못 가는지, 대신 어디로 가야 하는지를 이해할 수 있어야 함

### 디자인 검토 기준

- 디자인 검토는 항상 최신 디자인 규칙과의 차이를 기준으로 수행
- 표지판성 안내를 제외하고 의미 없는 숫자와 문구를 두지 않음
- 같은 역할의 버튼/카드/배지/숫자 정렬은 전 페이지에서 통일
- 승인 대기, 반려, 연결 대기, 재승인 대기 화면은 다음 행동과 이동 경로를 버튼 또는 링크로 즉시 보여줌
- 개발언어, 영어 시스템 문구, 내부 코드 용어, 개발자용 설명, 추상적인 업무 용어를 사용자 인터페이스에 직접 노출하지 않음
- 사용자가 꼭 알아야 하는 내용만 화면 문맥에 맞는 한국어로 다시 써서 보여줌
- 같은 의미의 안내 카드나 상태 박스는 나란히 놓였을 때 높이와 여백을 맞추고, 줄바꿈 때문에 글자가 깨지지 않게 구성함
- 내부 상태명, 영어 식별자, 개발자용 안내, 추상적인 운영 표현을 그대로 붙이지 말고 사용자가 바로 이해할 수 있는 한국어 제목과 설명으로 바꿔야 함
- 같은 맥락을 보여주는 요약 박스·상태 박스·보관함 박스는 크기와 정렬을 맞추고, 일부만 크게 또는 작게 보여 사용자 주의를 흐리지 않음
