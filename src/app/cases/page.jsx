...
function CasesContent() {
  ...
  const pushStateToUrl = useCallback(
    ({ nextSearchTerm, nextSearchType, nextPage, nextPageSize }) => {
      ...
      if ((nextPageSize || DEFAULT_PAGE_SIZE) !== DEFAULT_PAGE_SIZE) {
    ...
    try {
      ...
      const [partiesResult, clientsResult, lawsuitsResult] = await Promise.all([
        supabase
          ...
          .select(
            "id, case_id, party_type, entity_type, name, company_name, kcb_checked, kcb_checked_date, payment_notification_sent, payment_notification_date"
          )
          .in("case_id", caseIds),
        supabase
          .from("test_case_clients")
          .select("id, case_id, client_type, individual_id, organization_id")
          .in("case_id", caseIds),
        supabase
          .from("test_case_lawsuits")
          .select("id, case_id, lawsuit_type, court_name, case_number, created_at, updated_at")
          .in("case_id", caseIds)
          .order("created_at", { ascending: false }),
      ]);
      ...
      const userIds = [
        ...new Set(
          clientsData
            .filter((item) => item.client_type === "individual" && isUuid(item.individual_id))
      ...
      const organizationIds = [
        ...new Set(
          clientsData
            .filter((item) => item.client_type === "organization" && isUuid(item.organization_id))
      ...
      const [usersResult, organizationsResult] = await Promise.all([
        ...
        organizationIds.length > 0
          ? supabase.from("test_organizations").select("id, name").in("id", organizationIds)
          : Promise.resolve({ data: [], error: null }),
      ]);
      ...
      clientsData.forEach((client) => {
        if (!clientsByCase.has(client.case_id)) {
          clientsByCase.set(client.case_id, []);
        }
      ...
      lawsuitsData.forEach((lawsuit) => {
        if (!lawsuitsByCase.has(lawsuit.case_id)) {
          lawsuitsByCase.set(lawsuit.case_id, []);
        }
      ...
      const normalizedCases = casesData.map((caseItem) => {
        ...
        const creditor = caseParties.find((party) =>
          PARTY_ROLE_GROUPS.creditor.includes(party.party_type)
        );
        const debtor = caseParties.find((party) =>
          PARTY_ROLE_GROUPS.debtor.includes(party.party_type)
        );

        const clientNames = caseClients
          .map((client) => {
            if (client.client_type === "individual") {
              return userMap.get(client.individual_id)?.name || null;
            }
            if (client.client_type === "organization") {
              return organizationMap.get(client.organization_id)?.name || null;
            }
            return null;
          })
          .filter(Boolean);
  ...
  const filteredCases = useMemo(() => {
    ...
    const exactMatches = source.filter(
      (caseItem) => normalizeSearchText(caseItem.case_number) === normalizedTerm
    );
    ...
    return source.filter((caseItem) => {
      const searchable = [caseItem.case_number, caseItem.court_name, caseItem.id]
        .map(normalizeSearchText)
        .join(" ");
      return searchable.includes(normalizedTerm);
    });
  ...
  const getCaseSummaryText = (caseItem) => {
    ...
    if (caseItem.clientName && caseItem.clientName !== "미등록") {
  ...
  const PaginationComponent = ({ activePage, pages, onChange }) => {
    ...
    return (
      <div className="flex gap-1">
        <Button variant="outline" size="sm" disabled={activePage === 1} onClick={() => onChange(activePage - 1)}>
          이전
        </Button>
      ...
      <div className="mb-8">
        ...
        <Tabs value={searchType} onValueChange={handleTabChange} className="mt-4">
          ...
          <form onSubmit={handleSearchSubmit} className="flex w-full mb-6">
            ...
            <Button type="submit" className="ml-2" disabled={!searchTerm.trim() && !searchParams.get("q")}>
              검색
            </Button>
          </form>
          ...
          <Card className="border-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-xl overflow-hidden shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg">
                {searchTerm.trim()
                  ? `'${searchTerm.trim()}' 검색 결과 (${totalResults}건)`
                        ...
                        >
                          ...
                          <TableCell className="py-3">
                            <div className="flex flex-col">
                              <span className="text-sm truncate max-w-[240px]">
                                {getCaseSummaryText(caseItem)}
                              </span>
                              {searchType === "party_name" && (
                                <span className="text-xs text-muted-foreground mt-1">
                                  의뢰인: {caseItem.clientName}
                                </span>
                              )}
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="py-3">
                            <span className="font-medium text-sm md:text-base">
                              {formatUtil(caseItem.principal_amount || 0)}
                            </span>
                          </TableCell>
                          ...
                          <TableCell className="py-3">
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="text-sm text-gray-500">
                                {[caseItem.court_name, caseItem.case_number].filter(Boolean).join(" ") ||
                                  "등록된 소송 정보 없음"}
                              </div>
                            </div>
                          </TableCell>
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                ...
                                <DropdownMenuItem
                                  onClick={(event) => {
                                    event.stopPropagation();
                                    openCaseDetail(caseItem.id, "lawsuits");
                                  }}
                                  className="cursor-pointer"
                                >
                                ...
                                <DropdownMenuItem
                                  onClick={(event) => {
                                    event.stopPropagation();
                                    openCaseDetail(caseItem.id, "recovery");
                                  }}
                                  className="cursor-pointer"
                                >
              ...
              {!loading && totalResults > 0 && (
                <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-4 flex-col sm:flex-row">
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    총 {totalResults}개 중 {(currentPage - 1) * pageSize + 1}-
                    {Math.min(currentPage * pageSize, totalResults)}개 표시
                  </div>
                  <PaginationComponent
