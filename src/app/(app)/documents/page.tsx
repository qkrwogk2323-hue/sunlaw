  ...
  return (
    <div className="space-y-6">
      <div className="vs-brand-panel overflow-hidden rounded-[1.8rem] p-6 text-white shadow-[0_24px_54px_rgba(8,47,73,0.26)]">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-200/88">조직이 올린 사건 문서와 협업 공유 문서를 한곳에 모아 다시 열고 검토할 수 있게 구성했습니다.</p>
      </div>
      <div>
        <h2 className="text-2xl font-semibold tracking-tight text-slate-900">업로드 문서 목록</h2>
        <p className="mt-2 text-sm text-slate-600">조직이 올린 문서와 협업방에서 공유한 파일을 최근 순서대로 확인합니다.</p>
      ...
      <Card className="vs-mesh-card">
        ...
        <CardContent className="space-y-3">
          {documents.length ? documents.map((item: any) => (
            <Link key={item.id} href={(item.case_id ? `/cases/${item.case_id}` : `/api/documents/${item.id}/download`) as Route} className="vs-interactive block rounded-xl border border-slate-200 bg-white/85 p-4 transition hover:border-slate-900">
