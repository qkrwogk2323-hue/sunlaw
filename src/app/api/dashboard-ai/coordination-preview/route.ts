...
export async function POST(request: Request) {
  ...
  const preparedContent = prepareAiTextForFeature('case_hub_conversation', content);
  ...
  const preview = await buildCoordinationPlan(preparedContent.value, (cases ?? []) as Array<{ id: string; title: string }>);
