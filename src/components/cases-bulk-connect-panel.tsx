...
export function CasesBulkConnectPanel({ unlinkedClientCaseIds, unlinkedHubCaseIds }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 space-y-4">
      <p className="text-sm font-semibold text-amber-900">일괄 연동 가능한 사건이 있습니다.</p>
      ...
      {unlinkedHubCaseIds.length > 0 ? (
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500">각 사건 카드의 허브 연동 버튼으로 허브를 연결하세요. 허브 연동 후 사건허브 목록에 바로 나타납니다.</p>
        </div>
      ) : null}
    </div>
  );
