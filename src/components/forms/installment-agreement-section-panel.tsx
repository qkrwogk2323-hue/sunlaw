'use client';

import { useMemo, useState } from 'react';
import Link from 'next/link';
import type { Route } from 'next';
import { Pencil, Plus, Trash2 } from 'lucide-react';
import { addOrganizationFeeAgreementAction, deleteFeeAgreementAction, updateFeeAgreementAction } from '@/lib/actions/case-actions';
import { Badge } from '@/components/ui/badge';
import { Button, buttonStyles } from '@/components/ui/button';
import { ClientActionForm } from '@/components/ui/client-action-form';
import { DangerActionButton } from '@/components/ui/danger-action-button';
import { Input } from '@/components/ui/input';
import { SubmitButton } from '@/components/ui/submit-button';
import { Textarea } from '@/components/ui/textarea';
import { formatCurrency, formatDate } from '@/lib/format';
import { InstallmentFollowUpActions } from '@/components/forms/installment-follow-up-actions';

type PartyOption = { id: string; name: string };
type CaseOption = {
  id: string;
  title: string;
  clients: PartyOption[];
  organizations: PartyOption[];
};

type AgreementItem = {
  id: string;
  title: string;
  agreement_type: string;
  fixed_amount?: number | null;
  taxAmount?: number | null;
  totalAmount?: number | null;
  paidAmount?: number | null;
  shortageAmount?: number | null;
  effective_from?: string | null;
  effective_to?: string | null;
  description?: string | null;
  is_active: boolean;
  case_id?: string | null;
  bill_to_case_client_id?: string | null;
  bill_to_case_organization_id?: string | null;
  targetLabel: string;
  hub?: { id: string; title: string } | null;
  cases?: { title?: string | null } | Array<{ title?: string | null }> | null;
  terms_json?: Record<string, any> | null;
};

function relatedTitle(value?: { title?: string | null } | Array<{ title?: string | null }> | null) {
  if (Array.isArray(value)) return value[0]?.title ?? null;
  return value?.title ?? null;
}

function AgreementEditor({
  item,
  caseOptions
}: {
  item: AgreementItem;
  caseOptions: CaseOption[];
}) {
  const currentCase = caseOptions.find((option) => option.id === item.case_id) ?? caseOptions[0] ?? null;
  const [partyKind, setPartyKind] = useState<'case_client' | 'case_organization'>(item.bill_to_case_client_id ? 'case_client' : 'case_organization');
  const targetOptions = partyKind === 'case_client' ? (currentCase?.clients ?? []) : (currentCase?.organizations ?? []);
  const targetDefaultValue = partyKind === 'case_client'
    ? (item.bill_to_case_client_id ?? '')
    : (item.bill_to_case_organization_id ?? '');

  return (
    <ClientActionForm
      action={updateFeeAgreementAction.bind(null, item.id)}
      successTitle="분할납부 약정을 수정했습니다."
      errorTitle="분할납부 약정 수정에 실패했습니다."
      errorCause="입력값 또는 연결 대상 정보가 맞지 않습니다."
      errorResolution="사건, 대상, 금액을 다시 확인한 뒤 저장해 주세요."
      className="grid gap-3 md:grid-cols-2"
    >
      <input type="hidden" name="agreementType" value="installment_plan" />
      <select
        name="billToPartyKind"
        value={partyKind}
        onChange={(event) => setPartyKind(event.target.value as 'case_client' | 'case_organization')}
        className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900"
      >
        <option value="case_client">의뢰인</option>
        <option value="case_organization">참여 조직</option>
      </select>
      <select
        key={`${item.id}:${partyKind}:${targetDefaultValue}`}
        name={partyKind === 'case_client' ? 'billToCaseClientId' : 'billToCaseOrganizationId'}
        defaultValue={targetDefaultValue}
        className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900"
      >
        <option value="">대상 선택</option>
        {targetOptions.map((option) => (
          <option key={option.id} value={option.id}>{option.name}</option>
        ))}
      </select>
      {partyKind === 'case_client' ? <input type="hidden" name="billToCaseOrganizationId" value="" /> : <input type="hidden" name="billToCaseClientId" value="" />}
      <Input name="title" defaultValue={item.title} placeholder="약정명" required className="md:col-span-2" />
      <Input name="fixedAmount" type="number" min="0" step="0.01" defaultValue={String(item.fixed_amount ?? 0)} placeholder="공급가액" />
      <Input name="taxAmount" type="number" min="0" step="0.01" defaultValue={String(item.taxAmount ?? 0)} placeholder="부가세" />
      <Input name="effectiveFrom" type="date" defaultValue={item.effective_from ?? ''} />
      <Input name="effectiveTo" type="date" defaultValue={item.effective_to ?? ''} />
      <Textarea name="description" defaultValue={item.description ?? ''} placeholder="약정 설명" className="md:col-span-2" />
      <input type="hidden" name="termsJson" value={JSON.stringify(item.terms_json ?? {})} />
      <div className="md:col-span-2">
        <SubmitButton pendingLabel="수정 저장 중...">수정 저장</SubmitButton>
      </div>
    </ClientActionForm>
  );
}

export function InstallmentAgreementSectionPanel({
  caseOptions,
  items
}: {
  caseOptions: CaseOption[];
  items: AgreementItem[];
}) {
  const [createOpen, setCreateOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [selectedCaseId, setSelectedCaseId] = useState(caseOptions[0]?.id ?? '');
  const [partyKind, setPartyKind] = useState<'case_client' | 'case_organization'>('case_client');

  const selectedCase = useMemo(
    () => caseOptions.find((option) => option.id === selectedCaseId) ?? caseOptions[0] ?? null,
    [caseOptions, selectedCaseId]
  );
  const targetOptions = partyKind === 'case_client' ? (selectedCase?.clients ?? []) : (selectedCase?.organizations ?? []);

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-slate-900">분할납부약정금액</h2>
        <Button type="button" variant="secondary" size="sm" onClick={() => setCreateOpen((current) => !current)}>
          <Plus className="mr-1 size-4" />
          {createOpen ? '닫기' : '분납 약정 추가'}
        </Button>
      </div>

      {createOpen ? (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <ClientActionForm
            action={addOrganizationFeeAgreementAction}
            successTitle="분할납부 약정을 추가했습니다."
            errorTitle="분할납부 약정 추가에 실패했습니다."
            errorCause="사건, 대상, 금액 중 하나가 비어 있거나 맞지 않습니다."
            errorResolution="사건과 대상, 약정 금액을 다시 확인해 주세요."
            onSuccess={() => setCreateOpen(false)}
            className="grid gap-3 md:grid-cols-2"
          >
            <input type="hidden" name="agreementType" value="installment_plan" />
            <input type="hidden" name="termsJson" value='{"billing_intent":"installment_pending"}' />
            <select
              name="caseId"
              value={selectedCaseId}
              onChange={(event) => setSelectedCaseId(event.target.value)}
              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900"
            >
              {caseOptions.map((option) => (
                <option key={option.id} value={option.id}>{option.title}</option>
              ))}
            </select>
            <select
              name="billToPartyKind"
              value={partyKind}
              onChange={(event) => setPartyKind(event.target.value as 'case_client' | 'case_organization')}
              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900"
            >
              <option value="case_client">의뢰인</option>
              <option value="case_organization">참여 조직</option>
            </select>
            <select
              name={partyKind === 'case_client' ? 'billToCaseClientId' : 'billToCaseOrganizationId'}
              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 md:col-span-2"
            >
              <option value="">대상 선택</option>
              {targetOptions.map((option) => (
                <option key={option.id} value={option.id}>{option.name}</option>
              ))}
            </select>
            {partyKind === 'case_client' ? <input type="hidden" name="billToCaseOrganizationId" value="" /> : <input type="hidden" name="billToCaseClientId" value="" />}
            <Input name="title" placeholder="약정명" required className="md:col-span-2" />
            <Input name="fixedAmount" type="number" min="0" step="0.01" placeholder="공급가액" />
            <Input name="taxAmount" type="number" min="0" step="0.01" placeholder="부가세" />
            <Input name="effectiveFrom" type="date" />
            <Input name="effectiveTo" type="date" />
            <Textarea name="description" placeholder="약정 설명" className="md:col-span-2" />
            <div className="md:col-span-2">
              <SubmitButton pendingLabel="추가 중...">추가</SubmitButton>
            </div>
          </ClientActionForm>
        </div>
      ) : null}

      {items.length ? items.map((item) => (
        <div key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="font-medium text-slate-900">{item.title}</p>
              <p className="mt-1 text-sm text-slate-500">{relatedTitle(item.cases) ?? '사건'} · {item.targetLabel}</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone={item.shortageAmount ? 'amber' : 'blue'}>{item.shortageAmount ? '후속 필요' : '진행중'}</Badge>
              <Badge tone={item.is_active ? 'blue' : 'slate'}>{item.is_active ? '활성' : '비활성'}</Badge>
            </div>
          </div>
          <div className="mt-3 grid gap-2 text-sm text-slate-600 md:grid-cols-2">
            <p>공급가액 {formatCurrency(item.fixed_amount ?? 0)} · 부가세 {formatCurrency(item.taxAmount ?? 0)} · 합계 {formatCurrency(item.totalAmount ?? 0)}</p>
            <p>현재 입금 {formatCurrency(item.paidAmount ?? 0)} · 부족 {formatCurrency(item.shortageAmount ?? 0)}</p>
          </div>
          <div className="mt-2 text-xs text-slate-500">
            적용 {formatDate(item.effective_from)} ~ {formatDate(item.effective_to)}
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link href={`/cases/${item.case_id}?tab=billing`} className={buttonStyles({ variant: 'secondary', size: 'sm', className: 'h-9 rounded-xl px-3 text-xs' })}>사건에서 보기</Link>
            {item.hub?.id ? (
              <Link href={`/inbox/${item.hub.id}` as Route} className={buttonStyles({ variant: 'secondary', size: 'sm', className: 'h-9 rounded-xl px-3 text-xs' })}>허브에서 보기</Link>
            ) : null}
            {item.bill_to_case_client_id ? (
              <Link href={'/portal/billing' as Route} className={buttonStyles({ variant: 'secondary', size: 'sm', className: 'h-9 rounded-xl px-3 text-xs' })}>의뢰인 화면 보기</Link>
            ) : null}
            <Button type="button" variant="secondary" size="sm" onClick={() => setEditingId((current) => current === item.id ? null : item.id)}>
              <Pencil className="mr-1 size-4" />
              {editingId === item.id ? '수정 닫기' : '수정'}
            </Button>
            <DangerActionButton
              action={deleteFeeAgreementAction}
              fields={{ agreementId: item.id }}
              confirmTitle="이 분납 약정을 삭제할까요?"
              confirmDescription="삭제하면 분납 후속 처리와 비용 목록에서 제외됩니다."
              highlightedInfo={item.title}
              confirmLabel="삭제"
              buttonVariant="secondary"
              className="border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100"
            >
              <Trash2 className="mr-1 size-4" />
              삭제
            </DangerActionButton>
          </div>
          {(item.shortageAmount ?? 0) > 0 ? (
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
              <InstallmentFollowUpActions agreementId={item.id} caseId={item.case_id ?? ''} />
            </div>
          ) : null}
          {editingId === item.id ? (
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
              <AgreementEditor item={item} caseOptions={caseOptions} />
            </div>
          ) : null}
        </div>
      )) : (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-500">
          현재 등록된 분납 약정이 없습니다.
        </div>
      )}
    </div>
  );
}
