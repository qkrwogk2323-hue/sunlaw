...
- **주의**: 코드에 `자동차` case가 누락되어 `default: 70`으로 처리됨 — 실무 기준 50%로 추가 필요
...
> **주의**: 코드의 `PLAN_SECTION_LABELS`("제1항: 변제기간", "제2항: 변제방법", …)는 위 법원서식 제목과 불일치한다. `document-generator.ts`의 `generateRepaymentPlan()`은 올바른 법원서식 제목을 사용한다.
